...
def train():
    ...
    for ep in range(MAX_EPISODES):
        ...
        while True:
            ...
            s_, done, info = car_env.step_next(Carlist, time_tag, test_flag=True, action=a)
            ...
            if done:
                ...
                print('Ep:', ep, result, '| R: %i' % int(ep_reward), '| Explore: %.2f' % var, '| info: ', info,
                      '| dist-err(f1-f2):%.2f' % s[1], '| speed-err(f1-f2):%.2f' % s[0],
                      '| speed-err(le-f2):%.2f' % s[2])
                break
        # 画一下最后一次的图像
...
def eval():
    ...
    while True:
        ...
        # 多车同时加入仿真的计算
        Carlist[0].calculate(Carlist[0], STRATEGY='ACC', time_tag=time_tag, test_flag=False, action=None)  # 先算头车
        Carlist[1].calculate(Carlist[0:2], STRATEGY='ACC', time_tag=time_tag, test_flag=False, action=None)  # 先算第二辆
        for car_index in range(len(Carlist)):
            ...
            temp_list[2].calculate(temp_list, STRATEGY='RL', time_tag=time_tag, test_flag=False, action=a)  # 将输入的动作用于运算
