...
class car(object):
    ...
    # 构建测试场景
    def __test_scenario(self, TEST_SCENARIO, time_tag, test_flag):
        if not test_flag:
            return
    ...
    # 车辆运动学的主函数
    def calculate(self, CarList, STRATEGY, time_tag, test_flag, action=None):
        # 存储上次的数据
        ...
        if self.role == 'leader':
            ...
            if self.start_test == True:
                car.__test_scenario(self, test_method, time_tag, test_flag=test_flag)
            else:
                # 车辆跟驰
                if self.engaged_in_platoon:
                ...
                car.__test_scenario(self, test_method, time_tag,
                                    test_flag=test_flag)  # 因为__test_scenario包含了对是否到达测试地点的判断，所以需要在正常行驶的时候就检查一下
...
######################### car类相关的外部函数 #########################
# 计算运动学参数
def CarList_calculate(Carlist, STRATEGY, time_tag, test_flag, action):
    if STRATEGY == 'RL':
        if len(Carlist) >= 2:
            Carlist[1].calculate(Carlist, STRATEGY, time_tag, test_flag, action)  # RL时只算第二辆车
        else:
            return
    else:
        for single_car in Carlist:
            single_car.calculate(Carlist, STRATEGY, time_tag, test_flag, action)
...
# 根据动作值步进更新，仅用于RL
def step_next(Carlist, time_tag, test_flag, action):
    CarList_calculate(Carlist[0:2], STRATEGY='ACC', time_tag=time_tag, test_flag=test_flag,
                      action=None)  # 将输入的动作用于运算，先算前两辆
    CarList_calculate(Carlist[1:], STRATEGY='RL', time_tag=time_tag, test_flag=test_flag,
                      action=action)  # 将输入的动作用于运算，再算第三辆
    turns = 0
    done = False
    while turns <= AI_DT:
...
def get_reward_function(observation, post_jerk):
    ...
    if abs(post_jerk) <= turn_point1[0]:
        ...
        temp_r3 = (turn_point3[1] - turn_point2[1]) / (turn_point3[0] - turn_point2[0]) * (
            abs(post_jerk) - turn_point2[0]) + turn_point3[1]
